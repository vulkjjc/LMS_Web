							...
							<table class="table table-bordered" >
								<thead>
									<tr>
										...
										<th class="text-center">Info</th>
										<th class="text-center">Contact No.</th>
										<th class="text-center">Method/Due Amount</th>
										<th class="text-center">Laundry Item</th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td class=""><?php echo $row1['phone']; ?></td>
										<?php 
											$list2=$conn->query("SELECT * FROM payment WHERE order_id='".$row['id']."'");
											while($row2=$list2->fetch_assoc()):
										?>
										<?php if($row2['method']==1) ?>
										<td>COD-><?php echo $row['total_amount']; ?></td>
										<?php endwhile; ?>
										<td class=""><button class="btn btn-outline-primary btn-sm show" onClick=""  data-id="<?php echo $row['id'] ?>">Show Laundry</button></td>
									</tr>
									<?php endwhile; endwhile; ?>
